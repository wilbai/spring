                    ...
                    <table class="table table-hover">
                       ...
                       <%-- <c:if test="${disk.type == 'file'}">
                            <tr>
                                <td colspan="4">
                                    <c:choose>
                                        <c:when test="${disk.name.endsWith('.txt') or disk.name.endsWith('.pdf') or disk.name.endsWith('.jpg') or disk.name.endsWith('.png') or disk.name.endsWith('.bmp') or disk.name.endsWith('.gif')}">
                                            <a href="/disk/download?_=${disk.id}" target="_blank" class="btn btn-sm btn-info"><i class="fa fa-search"></i> 在线预览</a>
                                            &lt;%&ndash;<a href="/disk/download?_=${disk.id}&fileName=${disk.name}" class="btn btn-sm btn-danger"><i class="fa fa-download"></i> 下载</a>&ndash;%&gt;
                                        </c:when>
                                        <c:otherwise>
                                            <a href="/disk/download?_=${disk.id}&fileName=${disk.name}" class="btn btn-sm btn-danger"><i class="fa fa-download"></i> 下载</a>
                                        </c:otherwise>
    ...
    $(function () {
        ...
        $("#newFolder").click(function () {
            layer.prompt({title:"请输入文件夹名称"}, function (value, index) {
                layer.close(index);
                $.post("/disk/new", {"name":value,"pId":pId,"accountId":accountId}).done(function (json) {
